<x-layout>
    <div class="container px-4 2xl:pt-20">
                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <a href="#"
                                    class="text-sm font-medium text-gray-700 ms-1 hover:text-blue-600 md:ms-2 ">{{ $wisata->nama_wisata }}</a>
                            </div>
                        </li>
        ...
        <div class="">
            <div class="flex flex-col-reverse grid-cols-1 gap-4 xl:grid xl:grid-cols-8 xl:flex-col">
                <div class="xl:col-span-3 dm-sans rounded-xl">
                    ...
                    <div class="flex items-start space-x-2 font-semibold text-md">
                        <!-- Tombol "Deskripsi" -->
                        <button id="btn-deskripsi"
                            class="flex px-4 py-1 text-gray-500 transition-all duration-200 duration-500 ease-in-out bg-gray-200 tab-button active rounded-xl font-poppins hover:bg-gray-300 hover:text-gray-600 hover:scale-105 hover:shadow-lg hover:-translate-y-1 active:translate-y-0 active:scale-95">
                            Deskripsi
                        </button>

                        <!-- Tombol "Detail" -->
                        <button id="btn-detail"
                            class="flex px-4 py-1 text-gray-500 transition-all duration-200 duration-500 ease-in-out bg-gray-200 tab-button rounded-xl font-poppins hover:bg-gray-300 hover:text-gray-600 hover:scale-105 hover:shadow-lg hover:-translate-y-1 active:translate-y-0 active:scale-95">
                            Detail
                        </button>

                        <!-- Tombol "Rute" -->
                        <a href="{{ route('ruteTerdekat.index', $wisata->nama_wisata) }}" target="_blank"
                            class="flex px-4 py-1 text-gray-500 transition-all duration-200 duration-500 ease-in-out bg-gray-200 rounded-xl font-poppins hover:bg-gray-300 hover:text-gray-600 hover:scale-105 hover:shadow-lg hover:-translate-y-1 active:translate-y-0 active:scale-95">
                            Rute
                        </a>
                    </div>
                    ...
                    <div class="py-4 pr-4">
                        <div id="deskripsi">
                            <p
                                class="text-lg leading-loose text-justify text-gray-700 md:text-xl font-poppins indent-8 md:indent-12 first-letter:text-4xl first-letter:font-bold first-letter:text-gray-900 first-letter:mr-2 first-letter:font-serif">
                            ...
                            <div class="grid grid-cols-1 gap-4 text-gray-800">
                                ...
                                <div class="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
                                    ...
                                    @foreach (['senin', 'selasa', 'rabu', 'kamis', 'jumat', 'sabtu', 'minggu'] as $hari)
                                        ...
                                        @endphp
                                        <div class="p-2 border-2 rounded-lg border-slate-500 bg-gray-50">
                ...
                <div class="flex flex-col overflow-hidden rounded-lg xl:col-span-5 xl:rounded-2xl ">
                    <!-- Gambar Utama dan Galeri -->
                    <div class="col-span-4 overflow-hidden rounded-lg">
                        <!-- Gambar Utama -->
                        <div class="w-full">
                            ...
                            <div class="grid grid-cols-8">
                                @foreach ($imgDetails as $img)
                                    <img src="{{ asset('storage/' . $img) }}"
                                        class="event-cover w-full aspect-[1/1] cursor-pointer  border-transparent hover:border-blue-500 "
                                        alt="Gambar detail"
                                        onclick="changeMainImage('{{ asset('storage/' . $img) }}')">
                                @endforeach
                            </div>
                        @else
                            <div class="py-4 text-center text-gray-500 font-poppins">
        ...
        @if ($wisata->kuliners->isEmpty())
            <div class="p-10 mb-10 font-bold text-center text-gray-500 border rounded">
                Kuliner Kosong.
            </div>
        @else
            <div class="relative">
                <div class="w-full h-auto py-4 overflow-x-auto">
                    <div class="flex gap-4 min-w-max">
                        ...
                        @foreach ($wisata->kuliners as $kulinersss)
                            <button data-modal-target="modal-gambar-detail-wisata-{{ $kulinersss->id }}"
                                ...
                                class="relative flex-shrink-0 group" type="button">

                                <!-- Aspect ratio and image sizing adjustment -->
                                <div class="w-64 overflow-hidden rounded-lg h-80">
                                    <!-- Set a fixed size for image container -->
                                    <img src="{{ asset('storage/' . $kulinersss->gambar_kuliner) }}"
                                        alt="{{ $kulinersss->nama_kuliner }}"
                                        class="object-cover w-full h-full transition duration-300 ease-in-out transform group-hover:scale-105 group-hover:brightness-90">
                                </div>

                                <div class="absolute inset-0 flex items-center justify-center">
                                    <figcaption class="absolute z-10 w-full text-center bottom-4">
                                        <h1
                                            class="text-xs font-semibold tracking-wide text-white uppercase md:text-sm">
        ...
        @if ($wisata->events && $wisata->events->isNotEmpty())
            <!-- Swiper Container -->
            <div class="w-full pt-4 pb-4 mb-20 swiper-container">
                <!-- Wrapper for Slides -->
                <div class="swiper-wrapper aspect-[1920/720] rounded-lg overflow-hidden">
                        ...
                        <div class="flex items-center justify-center swiper-slide">
                            <button data-modal-target="img-event-{{ $event->id }}"
                                ...
                                <img src="{{ asset('storage/' . $event->img) }}"
                                    class="block object-cover w-full h-auto max-h-full mx-auto transition-all duration-300 filter grayscale hover:grayscale-0"
                                    alt="{{ $event->name ?? 'Event Image' }}">
                                <p class="mt-4 text-lg font-semibold text-center">
                                    {{ $event->nama_event ?? 'Event Name' }}</p>
                            </button>
                        </div>
                    @endforeach
                </div>
        ...
        @else
            <div class="p-10 mb-10 font-bold text-center text-gray-500 border rounded font-poppins">
            ...
            <div class="grid grid-cols-2 gap-4 xl:grid-cols-4 xl:gap-8">
                @foreach ($lainnya as $filter)
                    <a href="{{ route('profilWisata.index', $filter->nama_wisata) }}"
                        class="flex flex-col justify-between w-auto h-auto transition-transform duration-300 ease-in-out transform bg-white rounded-lg shadow-xl shadow-slate-400 border-slate-500 hover:scale-105 hover:shadow-2xl hover:shadow-slate-400">
                        <div>
                            ...
                            <div class="w-auto h-auto px-4 pt-2 pb-4">
                                <p class="font-bold text-black text-md">{{ $filter->nama_wisata }}</p>
            ...
            <div class="relative w-full max-w-2xl max-h-full p-4">
                ...
                <div class="relative bg-white rounded-lg shadow-sm ">
                    ...
                    <div class="p-4 space-y-4 md:p-5">
                        <img src="{{ asset('storage/' . $event->img) }}"
                            class="block object-contain w-full h-auto max-h-full mx-auto transition-all duration-300 filter grayscale hover:grayscale-0"
                            alt="{{ $event->name ?? 'Event Image' }}">
                    </div>
                </div>
            </div>
    ...
    @foreach ($wisata->kuliners as $kuliner)
        <div id="modal-gambar-detail-wisata-{{ $kuliner->id }}" tabindex="-1" aria-hidden="true"
            class="fixed inset-0 z-50 flex items-center justify-center hidden p-4 overflow-y-auto bg-black bg-opacity-50">
            <div
                class="relative w-full max-w-4xl overflow-hidden bg-white rounded-lg shadow-lg md:max-w-5xl xl:max-w-7xl">

                <!-- Tombol Close -->
                <button data-modal-hide="modal-gambar-detail-wisata-{{ $kuliner->id }}"
                    class="absolute z-20 flex items-center justify-center w-8 h-8 transition-all bg-white border border-gray-300 rounded-full top-4 right-4 hover:bg-gray-100">
                    ✕
                </button>
                ...
                <div class="grid gap-6 p-6 md:grid-cols-4">
                    <!-- Kolom Gambar -->
                    <div class="flex flex-col h-full space-y-4 md:col-span-2">
                        @if (!empty($kuliner->gambar_menu))
                            <div class="flex pb-4 space-x-4 overflow-x-auto flex-nowrap">
                                @foreach (json_decode($kuliner->gambar_menu, true) as $gambar_menu)
                                    <div class="flex-shrink-0 w-[225px] aspect-[3/4]">
                                        <button type="button"
                                            class="w-full h-full overflow-hidden transition-transform transform border rounded-lg border-slate-200 hover:shadow-xl hover:scale-105"
                                ...
                                <div>
                                    ...
                                    <ul class="space-y-2">
                                        @foreach ($jamOperasional as $jam)
                                            <li class="flex">
                                                <span
                                                    class="font-medium text-gray-600 w-28">{{ ucfirst($jam['hari']) }}:</span>
    ...
    <div id="imageModal" class="hidden fixed inset-0 z-[999] bg-black bg-opacity-90 flex items-center justify-center">
        ...
        <img id="modalImage" src="" class="object-contain max-w-full max-h-full rounded-lg"
            alt="Preview Gambar">
    </div>
    <script>
