<x-layadmin>
            ...
            <div id="kuliner-wrapper">
                <div class="kuliner-item border p-4 rounded mb-4">
                    ...
                    <div class="mb-2 font-poppins">
                        ...
                        <input type="tel" name="no_hp[]" class="w-full border p-2 rounded" required
                        ...
                        <div
                            ...
                            @foreach (['senin', 'selasa', 'rabu', 'kamis', 'jumat', 'sabtu', 'minggu'] as $index => $hari)
                                <div class="border-2 border-slate-500 p-3 rounded-lg bg-gray-50 relative">
                                    ...
                                    <input type="hidden" name="jam_operasional[0][{{ $index }}][hari]"
                                        value="{{ $hari }}">
                                    <div class="flex gap-2 time-inputs">
                                        <input type="time" name="jam_operasional[0][{{ $index }}][buka]"
                                            class="w-full p-2 border rounded text-sm font-poppins waktu-input"
                                            onchange="updateJamDisplay(this)">
                                        <input type="time" name="jam_operasional[0][{{ $index }}][tutup]"
                                            class="w-full p-2 border rounded text-sm font-poppins waktu-input"
                                            onchange="updateJamDisplay(this)">
                                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="file" name="gambar_menu[0][]" class="w-full border p-2 font-poppins"
                                multiple>
                        </div>
                    </div>
                    <div class="flex justify-end">
                        <button type="button"
                            class="btn-hapus bg-red-500 text-white px-3 py-1 rounded mt-2 font-poppins hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-blue-300 transition-all duration-200 ease-in-out hover:-translate-y-1 active:translate-y-0 active:scale-95">Hapus</button>
                    </div>
                </div>
            </div>

            <button type="button" id="tambah-kuliner"
                class="font-poppins w-full px-6 py-2 bg-green-600 text-white font-semibold rounded-lg shadow-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-blue-300 transition-all duration-200 ease-in-out hover:-translate-y-1 active:translate-y-0 active:scale-95">+
    ...
    <script>
        ...
        // Fungsi untuk update tampilan jam operasional
        ...
        // Tambahkan event listener untuk semua input waktu saat ini
        ...
        document.getElementById('tambah-kuliner').addEventListener('click', function() {
            ...
            clone.querySelectorAll('input[type="time"]').forEach(input => {
                input.value = '';
                // Tambahkan event listener baru untuk input waktu yang diklon
                input.addEventListener('change', function() {
                    updateJamDisplay(this);
                });
            });
            ...
            // Reset tampilan jam operasional untuk item baru
